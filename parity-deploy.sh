...
create_node_params() {
	...
	if [ ! -f $DEST_DIR/password ]; then
		...
		a=$(( b % 1000 ))
		pswds=$(( a * $1 ))
		echo $pswds"@dmin"$1 >$DEST_DIR/password
	fi
...
build_docker_config_poa() {
	...
	for x in $(seq 1 $CHAIN_NODES); do
		echo "version: '2.0'" >docker-compose-$x.yml
		echo "services:" >>docker-compose-$x.yml
		cat config/docker/authority.yml | sed -e "s/NODE_NAME/$x/g" | sed -e "s@-d /home/parity/data@-d /home/parity/data $PARITY_OPTIONS@g" >>docker-compose-$x.yml
		mkdir -p data/$x
		if [$x == $ETHSTATS]; then
			cat include/ethstats.yml >>docker-compose-$x.yml
		fi
		cat $DOCKER_INCLUDE >>docker-compose-$x.yml
		#expose_container $x
...
build_docker_config_ethstats() {
		...
		cat include/ethstats.yml >>docker-compose-$1.yml
...
expose_container() {
	...
	sed -i "s@container_name: $1@&\n       ports:\n       - 8080:8080\n       - 8180:8180\n       - 8545:8545\n       - 8546:8546\n       - 30303:30303@g" docker-compose-$1.yml
